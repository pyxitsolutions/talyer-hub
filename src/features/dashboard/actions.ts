...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  // Fallback: use paid invoices when sales_records are empty or stale
  if (monthlySales === 0) {
...
export async function getExpenseTrend(): Promise<TrendDataPoint[]> {
  const supabase = await createClient();
  const shopId = await getShopId();
  const months = getLastSixMonths();
...
export async function getProfitTrend(): Promise<TrendDataPoint[]> {
  ...
  return revenue.map((point, index) => ({
    month: point.month,
    value: point.value - (expenses[index]?.value ?? 0),
  }));
}

export async function getRepairCategories(): Promise<CategoryDataPoint[]> {
  const supabase = await createClient();
  const shopId = await getShopId();
  const monthStart = getMonthStart();
  const monthEnd = getMonthEnd();
...
export async function getTopSellingParts(): Promise<TopPartDataPoint[]> {
  const supabase = await createClient();
  const shopId = await getShopId();
  const monthStart = getMonthStart();
  const monthEnd = getMonthEnd();
